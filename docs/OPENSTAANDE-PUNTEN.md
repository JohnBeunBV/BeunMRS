...
> _"We missen de traceerbaarheid vanuit de requirements naar jullie ontwerp en realisatie. Ook de relatie met het testen ontbreekt."_
...
> _"Informatie is niet consistent: de verschillende niveau's van het C4-model."_
...
> _"Jullie hebben meer architectuurbeslissingen, toch?"_
...
> _"De FMEA is dun uitgewerkt en de doorwerking is niet terug te zien in de architectuur, realisatie en testen."_
...
> _"Toon aan welke requirements daadwerkelijk behaald zijn en hoe je dat weet."_
...
> _"Multitenancy/SaaS niet goed begrepen."_
> _"Uitbreiden met een nieuwe provider, geen goed beeld hoe dat moet buiten klasse aanmaken."_
> _"Systematische aanpak te mager."_
...
**Wat we hebben:** traceerbaarheidsmatrix + 10 ADR's + C4. De _inhoud_ is er. We zakten op **overtuigend kunnen toelichten**.

**Acties:**

1. Maak een **presentatie-rode draad** die één requirement end-to-end volgt, live: bijv. **FR-1g (annulering stopt reminders)** → ADR-004 (waarom RabbitMQ/event-driven) → `AppointmentEventConsumer.onCancellation()` → `cancelReminders()` → `AppointmentEventConsumerTest` → demo live een afspraak annuleren en `status='cancelled'` tonen. Dit is _de traceerbaarheid die ze misten_, live gedemonstreerd.
...
**Opgelost (2026-06-25):** Sectie _OpenMRS-versiecompatibiliteit (NFR-4)_ toegevoegd aan
ADR-003, tussen de FHIR-noot en de implementatiedetails. De sectie onderbouwt dat de module
uitsluitend stabiele `/ws/rest/v1/`-endpoints gebruikt en dat het compatibiliteitsrisico
structureel beperkt is tot de uitwisselbare poller-laag. Traceerbaarheidsmatrix NFR-4
bijgewerkt naar ✅ met verwijzing naar ADR-003 § OpenMRS-versiecompatibiliteit.
...
De docent: _"In de presentatie zien we dit ook terug."_ De documenten zijn nu goed; de **presentatie moet ze laten zien**.
...
1. _Vergelijk een aangereikt alternatief ontwerp met het jouwe — voor- én nadeel voor een gegeven NFR._
2. _Schets-toekomstige OpenMRS-ontwikkeling — hoe werk je dat uit vanuit het huidige ontwerp, incl. herontwerp?_
...
- **Alternatief dat ze kunnen voorleggen:** _database-per-tenant_ i.p.v. _shared-schema met tenant_id-kolom_.
...
5. **`NotificationProviderContractTest`** dekt hem automatisch (uniqueness, @Component, schema-match) — leg uit dat dit de uitbreiding _afdwingt_, niet alleen toelaat.
