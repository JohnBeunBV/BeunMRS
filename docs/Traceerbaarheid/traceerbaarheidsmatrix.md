...
| **FR-1f** — Sla notificatie over als afspraak al gestart is | ADR-004                   | `ReminderDispatchJob.processReminder()` — controleert `send_at < NOW()` vóór dispatch; zet status op `skipped`                                    | `ReminderDispatchJobTest` — `dispatch_appointmentAlreadyStarted_skipsAndDoesNotDispatch` (skip + status `skipped` bij `appointmentTime < now`)                                                                                | ✅     |
...
| **NFR-4** — OpenMRS 2.7.x compatibiliteit                              | ADR-003          | Gebruikt uitsluitend `/ws/rest/v1/` endpoints die stabiel zijn sinds OpenMRS 2.x; geen FHIR2 (zie HAPI-0302 notitie in ADR-003)                               | ADR-003 § OpenMRS-versiecompatibiliteit; `docs/Tests/postmanrequests.md` stap 6 + stap 7                                                   | ✅                                           |
...
| **NFR-6a** — HL7 berichtontvangst + validatie                          | ADR-004          | Jackson-gebaseerde deserialisatie van `AppointmentEvent` met strict type-mapping; RabbitMQ `MessageConverter` valideert JSON-structuur                        | `AppointmentEventConsumerTest` — test event-deserialisatie en null-afhandeling (ongeldige/onvolledige berichten worden veilig afgehandeld) | ✅                                           |
...
| Categorie                                                        | Detailregels in matrix | ✅ Bewezen |
| ---------------------------------------------------------------- | ---------------------- | ---------- |
| Functionele eisen (FR-1a … FR-3)                                 | 10                     | 10         |
| Niet-functionele eisen (NFR-1 … NFR-13, incl. sub-eisen)         | 23                     | 23         |
| **Totaal**                                                       | **33**                 | **33**     |

> **Telling:** de opdracht definieert **23 requirement-ID's** (10 FR + 13 NFR-hoofdnummers). In deze matrix zijn de NFR-hoofdnummers uitgesplitst naar toetsbare sub-eisen (NFR-2a-c, 5a-d, 6a-e, 9a-b), samen met de 10 FR-regels goed voor **33 detailregels — alle 33 ✅**.
